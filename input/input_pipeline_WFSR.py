...
flags.DEFINE_integer('OUTPUT_C', 1, 'The channel of output image [1]')     # reconstruction network for water/fat
flags.DEFINE_integer('LABEL_C', 2, 'The channel of label image [2]')       # label channel
flags.DEFINE_integer('CROP_SIZE', 64, 'The size of crop box [64]')
FLAGS = flags.FLAGS


def inpute(data_dir, type='train', label_type='water'):
    ...
    for fileidx in range(sample_num):
        ...
        if label_type == 'water':
           label_sets[fileidx, :, :, :] = data_pairs[:, :, 2:3]  # water channel:3
        else:
           label_sets[fileidx, :, :, :] = data_pairs[:, :, 3:4]  # fat channel:4
    print("finished\n")
